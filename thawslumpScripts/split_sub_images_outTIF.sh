...
do
${eo_dir}/grss_data_fusion/split_image.py -f GTiff -W ${patch_w} -H ${patch_h}  -e ${overlay} -o  ${root}/${test_dir}/split_images $img
done
for img in ${labelImg_dir}/*.tif
do
${eo_dir}/grss_data_fusion/split_image.py -f GTiff -W ${patch_w} -H ${patch_h}  -e ${overlay} -o ${root}/${test_dir}/split_labels $img
done
