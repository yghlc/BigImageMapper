...
def clip_vector_and_remove_tiny_polygons_touch_edge(input_vector, save_path, extent, area_thr_ratio=0.5,
                                class_column="class_int", background_class=0, format='ESRI Shapefile'):
    """
    ...
    for idx, row in gdf.iterrows():
        ...
        if org_area == 0:
            raise ValueError(f'area is 0: {row}')
        area_ratio = clipped_area / org_area
        if area_ratio < area_thr_ratio:
            removed_edge += 1
            continue
        if row[class_column] == background_class:
            removed_bg += 1
            continue
        ...
        clipped_rows.append(updated_row)
...
def get_bounding_boxes_from_vector_file(img_path_list, vector_path, b_ignore_edge_objects=False, b_save_removal=False):
    ...
    for idx, img_path in enumerate(img_path_list):
        ...
        # if the txt exist, skip
        if os.path.isfile(save_box_txt):
            save_box_txt_list.append(save_box_txt)
            continue

        if not os.path.isfile(save_crop_vector):

            out_path = clip_vector_and_remove_tiny_polygons_touch_edge(vector_path, save_crop_vector, raster_bounds, area_thr_ratio=0.5,
                                                            format='GPKG')
            if out_path is None:
            ...
            #     out_path = vector_gpd.remove_polygon_boxes_touch_edge(save_crop_vector,raster_bounds, shrink_meters=img_res_x,
            #                                                b_save_removal=b_save_removal)
            #     # if no geometries after removing those touching edge, saved an empty file txt needed by YOLO
            #     if out_path is None:
            #         io_function.save_list_to_txt(save_box_txt, [])
            #         save_box_txt_list.append(save_box_txt)
            #         continue
