...
# use python3 for running this script
export PATH=~/programs/anaconda3/bin:$PATH
merge_shp_py=~/codes/PycharmProjects/ChangeDet_DL/dataTools/merge_shapefiles.py
...
################################################
## merge shape file using python
${merge_shp_py} ${tile_dir}


################################################
# merge shape file using bash
#function merge_shp() {
#    out_shp=$1
#    out_name=$2
#    i=$3
#
#    # the first shape file cannot be empty, otherwise, all the field in the following would be skipped
#    # need to add script to check
#
#    if [ -f "$out_shp" ]
#    then
