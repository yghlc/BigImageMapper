...
def segment_rs_image_sam(image_path, save_dir, #model, model_type, 
                         sam2_checkpoint, model_cfg,
                         patch_w, patch_h, overlay_x, overlay_y,   
    ...
    # print('\n--------------------------------------------------------------------')
    basic.outputlogMessage('loading a checkpoint: %s'%sam2_checkpoint)
    ...
    #     sam2_model = build_sam2(model_type, model, device=device)
    # else:
    #     #  
    #     pass
    ...
    if prompts is None:
        ...
        # segment everything
        mask_generator = SAM2AutomaticMaskGenerator(sam2_model, output_mode="coco_rle")  # "binary_mask" take a lot of memory
    else:
        # only segment targets
        mask_generator = SAM2ImagePredictor(sam2_model)
...
def segment_remoteSensing_image(para_file, area_ini, image_path, save_dir, network_ini, batch_size=1):
    ...
    # model_cfg = parameters.get_string_parameters_None_if_absence(network_ini, 'model_cfg')
    # sam2_checkpoint = "../checkpoints/sam2.1_hiera_tiny.pt"
    # model_cfg = "configs/sam2.1/sam2.1_hiera_t.yaml"  # hard code here, should be the same as the model_cfg
    
    
    model_cfg = parameters.get_string_parameters_None_if_absence(network_ini, 'model_cfg')
    sam2_checkpoint = parameters.get_string_parameters_None_if_absence(network_ini, 'sam2_checkpoint')
    
    # print(1, model_cfg, sam2_checkpoint)
    # print(len(sam2_checkpoint), len(model_cfg))
    # for i in sam2_checkpoint:
    #     print(i, end=' ')
    # print('\n------------------------')
    # sam2_checkpoint1 = "../checkpoints/sam2.1_hiera_tiny.pt"
    # model_cfg1 = "configs/sam2.1/sam2.1_hiera_t.yaml"
    # print(2, model_cfg, sam2_checkpoint)
    # print(len(sam2_checkpoint1), len(model_cfg1))
    # for i in sam2_checkpoint1:
    #     print(i, end=' ')
    
    # print('\n------------------------')
    # print(sam2_checkpoint==sam2_checkpoint1,  model_cfg==model_cfg1)
...
def main(options, args):
    ...
    model_type = model_cfg.split('/')[-1].split('.yaml')[0]
    with open("time_cost.txt", 'a') as t_obj:
        t_obj.writelines('--------------------------------------------------\n')
        t_obj.writelines("Segment Anything model : %s\n" % model_type)
    ...
    with open("time_cost.txt", 'a') as t_obj:
        t_obj.writelines('\n************************************************\n')
        t_obj.writelines("Segment Anything model finished\n")
        t_obj.writelines("Start time: %s \n" % str(datetime.fromtimestamp(start_time, tz=timezone(timedelta(hours=8))).strftime('%Y-%m-%d %H:%M:%S')))
        t_obj.writelines("End time: %s \n" % str(datetime.fromtimestamp(end_time, tz=timezone(timedelta(hours=8))).strftime('%Y-%m-%d %H:%M:%S')))
        t_obj.writelines("Total time cost: %.2f seconds \n" % (end_time - start_time))
        t_obj.writelines('--------------------------------------------------\n')
...
if __name__ == '__main__':
    ...
    # time cost_file = 'time_cost.txt'
    # if os.path.isfile(time_cost_file):
    #     with open(time_cost_file, 'a') as f_obj:
    #         f_obj.writelines('Total time cost: %.2f seconds\n' % (end_time - start_time)) 
